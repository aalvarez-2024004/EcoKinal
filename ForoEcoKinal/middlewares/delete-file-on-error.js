            ...
            try {
                if(res.statusCode >= 400) {
                    ...
                    if(publicId){
                        ...
                        console.log(
                            `Imagen de publicación de Cloudinary eliminado por respuesta ${res.statusCode}: ${publicId}`
                        )
                    }
                }
            } catch (error) {
    ...
    try {
        if(req.file) {
            ...
            if(publicId){
                await cloudinary.uploader.destroy(publicId);
                console.log(
                    `Imagen de publicación de Cloudinary eliminado por error en cadena: ${publicId}`
                )
            }
        }
    } catch (error) {
